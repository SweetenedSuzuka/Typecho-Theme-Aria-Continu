...
<div id="main" class="col-mb-12 col-8 col-offset-2">
	...
	<div class="error-page">
		<?php
        $notFoundTitle = '404:没有找到界面呢，是书架摆错了吗？';
        if (isset($this->options->notFoundTitle)) {
            $value = trim((string) $this->options->notFoundTitle);
            if ($value !== '') {
                $notFoundTitle = $value;
            }
        }
		...
		<p>
			<?php
            $notFoundDescription = '这个页面不存在或者被删除，你可以尝试搜索你想要的内容。';
            if (isset($this->options->notFoundDescription)) {
                $value = trim((string) $this->options->notFoundDescription);
                if ($value !== '') {
                    $notFoundDescription = $value;
                }
            }
