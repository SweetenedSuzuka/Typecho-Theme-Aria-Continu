...
## 更新  
### 还没做完！
* 新增一批“原版没有、但常见站点会需要”的可配置项（均在后台主题设置中管理）：  
  * 页脚站点名称与链接（字段名：`footerSiteName` / `footerSiteUrl`）：用于替代页脚里硬编码的站点名与跳转地址  
  * 页脚署名显示方式（字段名：`footerCreditsMode`）：可选“原主题署名 / 自定义署名 / 隐藏署名”  
...
* 首页/搜索/列表相关增强：  
  * 首页副标题可配置（字段名：`heroSubtitle`）：用于自定义首页标题下方的副标题显示  
  * 搜索框 placeholder 可配置（字段名：`searchPlaceholder`）
  * 首页文章分类过滤规则可配置（字段名：`homeExcludeCategories`）并支持开关（字段名：`homeExcludeCategoriesEnabled`）
* 新增自定义文案配置：  
  * 评论等待审核提示（字段名：`commentWaitingText`）
  * 评论关闭提示（字段名：`commentClosedText`）
  * 404 标题与描述（字段名：`notFoundTitle` / `notFoundDescription`）
* 修复原版 MathJax 的失效问题，并升级为新版稳定加载方案：  
  * 仅在启用 MathJax 时加载资源  
  * 更换为 MathJax v4.1.2
